...
The v1.2.16 update makes revision merging atomic: it checks the original draft, writes the merged text, and marks the revision merged in one database transaction. Bulk character renaming now covers the complete roster in small batches and retries smaller batches when the model output hits its length limit; the preview opens only after every mapping is complete. Windows x64 and both macOS installers are distributed in the same desktop GitHub Release. The DSH plugin tarball remains available in the v1.2.0 Release; npm publication remains out of scope.

[Download v1.2.16](https://github.com/shuishuipingan/InkWeaver/releases/tag/v1.2.16) · [Start your first chapter in three minutes](docs/quickstart/README.md) · [Install the DSH plugin](plugins/inkweaver-dsh/README.md) · [Ask a question or report a problem](https://github.com/shuishuipingan/InkWeaver/issues/new/choose) · [Join the discussion](https://github.com/shuishuipingan/InkWeaver/discussions)
...
The current macOS installers do not have a Developer ID signature and are not notarized. Download only from the [formal v1.2.16 Release](https://github.com/shuishuipingan/InkWeaver/releases/tag/v1.2.16) and follow the operating system's first-launch confirmation. The desktop Release uses a seven-asset contract covering macOS Apple Silicon and macOS Intel; the DSH plugin tarball remains in the v1.2.0 Release.
