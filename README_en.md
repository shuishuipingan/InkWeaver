...
The v1.2.10 Windows x64, macOS Apple Silicon, and macOS Intel installers have passed qualification and are available in one GitHub Release. The DSH plugin tarball remains available in the v1.2.0 Release; npm publication remains out of scope.
...
The current macOS installers do not have a Developer ID signature and are not notarized. Download only from the [formal v1.2.10 Release](https://github.com/shuishuipingan/InkWeaver/releases/tag/v1.2.10) and follow the operating system's first-launch confirmation. The desktop Release uses a seven-asset contract covering macOS Apple Silicon and macOS Intel; the DSH plugin tarball remains in the v1.2.0 Release.
