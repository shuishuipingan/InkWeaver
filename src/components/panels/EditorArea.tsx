...
export default function EditorArea({ onNewProject }: EditorAreaProps) {
  ...
  return (
    ...
    >
        ...
        >
          {tabs.map((tab) => (
            <div
              ...
              className="flex items-center gap-1.5 px-3 h-full text-sm cursor-pointer group flex-shrink-0 relative transition-colors editor-tab"
              style={{
            ...
            >
              ...
              {tab.dirty ? (
                ...
                <button
                  className="opacity-0 group-hover:opacity-100 ml-0.5 p-0.5 rounded transition-colors hover:bg-[var(--color-hover)]"
      ...
      {/* 编辑区主体 */}
      <div className="flex-1 overflow-hidden">
        {/* Tab 内容 — ViewTransition 统一「退场 + 进场」转场（仅 opacity/transform，遵守 prefers-reduced-motion） */}
        <ViewTransition transitionKey={activeTab?.id ?? 'none'} className="h-full">
