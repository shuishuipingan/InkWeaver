import type { MessageKey } from './en-US'

export const zhCN = {
  'appearance.anime': '手绘幻想',
  'appearance.animeDescription': '原创日系动画氛围，兼顾背景层次与文字可读性。',
  'appearance.change': '更换图片',
  'appearance.choose': '选择图片',
  'appearance.classic': '经典',
  'appearance.classicDescription': '原始界面，保持不变。',
  'appearance.custom': '自定义图片',
  'appearance.customAvailable': '当前图片已可使用。',
  'appearance.customHint': 'PNG / JPEG · 最大 20 MB · 建议 16:10',
  'appearance.customUnavailable': '选择一张图片来个性化工作台。',
  'appearance.remove': '移除',
  'appearance.section': '外观',
  'appearance.skins': '界面皮肤',
  'appearance.skinsDescription': '皮肤与颜色主题彼此独立，且不会改变可读性设置。',
  'appearance.theme': '主题',
  'appearance.themeDescription': '选择标题栏和界面控件使用的颜色主题。',
  'app.windowTitle': '织墨 — InkWeaver',
  'common.backup': '备份',
  'common.close': '关闭',
  'common.export': '导出',
  'common.maximizeRestore': '最大化/还原',
  'common.minimize': '最小化',
  'common.new': '新建',
  'common.open': '打开',
  'common.restore': '恢复',
  'common.settings': '设置',
  'language.chinese': '简体中文',
  'language.english': 'English',
  'language.settingDescription': '选择应用界面、系统提示和错误信息使用的语言。',
  'language.settingLabel': '应用语言',
  'language.switch': '切换界面语言',
  'error.knowledgeBaseNativeUnavailable': '知识库不可用：Windows 原生组件加载失败。请重新安装或下载完整发布包。',
  'error.legacyVectorMigrationBlocked': '旧版知识库数据需要先修复后才能继续。请修正项目中的 vectors.json，然后重试。',
  'error.projectNotOpen': '请先打开项目。',
  'error.embeddingModelNotConfigured': '请先配置向量模型。',
  'error.projectStoragePathUnsupported': '项目路径过深，部分本地存储不可用。请将整个项目文件夹移动到更靠近磁盘根目录的位置（例如 D:\\Novels），然后重试。',
  'error.projectRootRequired': '请选择项目根目录，也就是包含 .vela/project.json 的小说文件夹；不要选择它的上级文件夹。',
  'error.unknown': '发生错误：{message}',
  'project.backupUnavailable': '创建本地一致性快照',
  'project.backupSuccess': '快照已创建：{id}',
  'project.backupFailed': '快照创建失败：{error}',
  'project.restore': '恢复最新快照',
  'project.restoreUnavailable': '当前还没有可用的本地快照。',
  'project.restoreConfirm': '要把快照 {id} 恢复到新目录“{path}”吗？当前作品不会被修改。',
  'project.restoreSuccess': '快照已恢复到：{path}',
  'project.restoreFailed': '快照恢复失败：{error}',
  'project.current': '当前项目：{name}',
  'project.currentLabel': '当前项目：',
  'project.export': '导出',
  'project.imitation': '小说拆解与仿写',
  'project.imitationShort': '拆解仿写',
  'project.new': '新建项目',
  'project.none': '未打开项目',
  'project.open': '打开项目',
  'project.switch': '切换项目',
  'save.dirty': '有未保存的修改',
  'save.modified': '有修改',
  'save.saved': '已保存',
  'settings.editor.language': '语言',
  'theme.dark': '黑夜',
  'theme.galaxy': '星空',
  'theme.label': '主题：{name}',
  'theme.light': '浅色',
  'theme.paper': '纸质',
  'zoom.in': '放大',
  'zoom.out': '缩小',
  'zoom.reset': '重置缩放',
} satisfies Record<MessageKey, string>
