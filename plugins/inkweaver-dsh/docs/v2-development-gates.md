...
本文件只记录 InkWeaver DSH Extension 已经遇到的运行时陷阱和相应门禁。DeepSeek Harness 通用安装、固定上游基线与动态 Cordis Package 的说明仍由 [official-dsh-plugin-installation.md](official-dsh-plugin-installation.md) 负责；不要把同一套规则复制到多个地方。
