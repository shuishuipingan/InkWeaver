...
const templateRoot = join(packageRoot, 'presets', 'inkweaver')
...
describe('installed 织墨 preset session', () => {
  it('is discovered from the user root and gives one agent only the two novel tools', async () => {
    ...
    expect(listed).toContainEqual(expect.objectContaining({ id: 'inkweaver', trust: 'user' }))
    const handle = await ctx.agents.create({
      sessionId: SessionId('ai-novel-isolated'),
      setup: async agentCtx => void await ctx.agentPresets.mount(agentCtx, 'inkweaver'),
    })
  ...
  it('isolates an existing blank agent after the Web surface selects the novel preset', async () => {
    ...
    await ctx.agentPresets.recompose(handle.agent.ctx, 'inkweaver')
    ...
    handle.agent.session.append('agent-preset/selected', { agentPreset: 'inkweaver' })
  ...
  it('gives the independent V2 preset only the read-and-proposal tool surface', async () => {
    ...
    const listed = await ctx.agentPresets.list()
    expect(listed.map(preset => preset.id)).toContain('inkweaver-v2')
    expect(listed.map(preset => preset.id)).toContain('inkweaver')
    const handle = await ctx.agents.create({
      sessionId: SessionId('ai-novel-v2-isolated'),
      setup: async agentCtx => void await ctx.agentPresets.mount(agentCtx, 'inkweaver-v2'),
    })
    ...
    const protocol = assembly.sections.find(section => section.name === 'inkweaver-v2:proposal-protocol')?.text
    ...
    expect(v1Assembly.sections.some(section => section.name === 'inkweaver-v2:proposal-protocol')).toBe(false)
    await ctx.fiber.dispose()
  })
})
