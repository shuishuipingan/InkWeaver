...
describe('dsh-web-all installed-profile composition', () => {
  realProfileIt('keeps only the two novel tools in every real request header', async () => {
    ...
    const pluginScope = join(workspace, 'node_modules', '@shuishuipingan')
    await mkdir(pluginScope, { recursive: true })
    const pluginLink = join(pluginScope, 'inkweaver-dsh')
    if (!existsSync(pluginLink)) await symlink(packageRoot('@shuishuipingan/inkweaver-dsh'), pluginLink, 'junction')
    const entries = {
