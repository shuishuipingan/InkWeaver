...
const templateRoot = resolve(import.meta.dirname, '..', 'presets', 'inkweaver')
...
describe('织墨 preset installer', () => {
  it('installs the exact package assets and repeats idempotently', async () => {
    ...
    await expect(readFile(join(root, 'inkweaver', 'agent.cordis.yml'), 'utf8'))
      .resolves.toBe(await readFile(join(templateRoot, 'agent.cordis.yml'), 'utf8'))
    await expect(readFile(join(root, 'inkweaver', 'preset.yml'), 'utf8'))
  ...
  it('reports a same-name conflict without changing any user byte', async () => {
    ...
    const target = join(root, 'inkweaver')
  ...
  it('installs the independent V2 preset beside the unchanged V1 preset', async () => {
    ...
    expect((await readdir(root)).sort()).toEqual(['inkweaver', 'inkweaver-v2'])
    await expect(readFile(join(root, 'inkweaver', 'agent.cordis.yml'), 'utf8'))
      .resolves.toBe(await readFile(join(bundledPresetRoot, 'inkweaver', 'agent.cordis.yml'), 'utf8'))
    const v2Agent = await readFile(join(root, 'inkweaver-v2', 'agent.cordis.yml'), 'utf8')
    expect(v2Agent).toContain("name: '@shuishuipingan/inkweaver-dsh/agent-v2'")
  ...
  it('does not publish a preset when cancellation wins before installation', async () => {
    ...
    await expect(access(join(root, 'inkweaver'))).rejects.toThrow()
  })
})
