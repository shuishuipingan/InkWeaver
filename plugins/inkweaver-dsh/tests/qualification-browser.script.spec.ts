...
describe('V2 browser qualification journey', () => {
  ...
  it('accepts the V2 preset only when the real agent preset roster exposes it', async () => {
    const response = {
      ...
      result: {
        ok: true,
        value: {
          presets: [{ id: 'inkweaver-v2', trust: 'user', isDefault: true }],
          authorable: true,
          hasDocument: true,
        },
      },
    ...
    expect(JSON.parse(result.stdout)).toEqual({
      presetId: 'inkweaver-v2',
      presetIds: ['inkweaver-v2'],
    })
  })
  ...
  it('fails before opening the preset picker when the agent preset roster lacks V2', async () => {
    ...
    await expect(execFileAsync(process.execPath, [
      ...
      stderr: expect.stringContaining('agentPreset.list did not expose inkweaver-v2'),
    })
  })
