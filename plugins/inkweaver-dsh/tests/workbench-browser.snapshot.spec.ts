...
describe('AI novel real DSH Web snapshot', () => {
  it('keeps independent V1 generation/reconciliation and V2 read-only fixture tool faces on their own Presets', async () => {
    ...
    expect(v1Events[0]).toMatchObject({ type: 'session', agentPreset: 'inkweaver' })
    ...
    expect(v1Driver).toContain("default: 'inkweaver'")
    ...
    expect(v2Events[0]).toMatchObject({ type: 'session', agentPreset: 'inkweaver-v2' })
    ...
    expect(v2Driver).toContain("default: 'inkweaver-v2'")
