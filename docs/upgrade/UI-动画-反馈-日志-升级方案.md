# 织墨 / InkWeaver — UI、反馈与日志实施记录

> 文档版本：v1.0
> 分析基线：InkWeaver 源码树
> 升级范围：① UI 界面统一 ② 交互动画完善 ③ 用户反馈机制增强 ④ 日志系统完善
> 交付终点：构建打包并验证 InkWeaver Windows 安装目录
...
#   产物：release/1.0.0/win-unpacked/

# 4. 覆盖安装
#    关闭正在运行的 InkWeaver.exe
#    release/1.0.0/win-unpacked/* → 独立的 InkWeaver 验证目录
...
- 验证目录的结构与 `win-unpacked/` 完全一致（`InkWeaver.exe` / `resources/` / `locales/` / `*.dll` / `Uninstall InkWeaver.exe`）
- **必须先关闭正在运行的 `InkWeaver.exe`**，否则 `InkWeaver.exe` 与 `resources/` 会被占用导致覆盖失败
- `logs/` 目录为运行时生成，**覆盖时跳过**（保留历史日志）
- 覆盖后确认：`InkWeaver.exe` 时间戳更新、`resources/app.asar` 更新、版本为 1.0.0
