...
| G03 | 安装包与分发 | 13 | Windows 和两种 macOS 架构同 SHA 资格通过吗？ | 开发中 | 已回读 Windows 资格计划：`test → prepare:native-node → clean:build → build:win:artifacts → verify update/package → smoke app/installer/upgrade → restore native-node → final`；Windows release-contract 测试现为 22 passed/1 skipped，首轮 Vite 冷启动预算与 monitor step-complete 确认窗口已加固；macOS ARM64/x64 workflow 与统一提升合同已存在；当前尚未在 1.1.0 冻结 SHA 上构建或发布任何安装包 |
