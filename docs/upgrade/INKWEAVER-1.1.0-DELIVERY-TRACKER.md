...
| C01 | 全文人物提取 | 03 | 后半段、跨块代词、多章和导入文本是否都覆盖？ | 开发中 | `43e5da1`：定稿人物状态提取保留开头+结尾有界窗口；`f03503c`：source-bound 全文分块计划；`f94a927`：证据字段解析、跨块别名合并和冲突标记；`edd74ce`：冻结模型租约的多分块提取工作流；候选 UI/确认提交未完成 |
