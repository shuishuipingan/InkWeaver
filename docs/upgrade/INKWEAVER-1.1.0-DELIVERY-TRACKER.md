...
| E05 | 任务恢复 | 08 | 取消、重启、重复请求是否不造成重复或错误写入？ | 开发中 | 工作流新增不含正文的恢复收据：冻结项目会话、步骤状态、进度、失败码和安全边界写入本地 checkpoint；跨 lease 自动拒绝恢复；取消/暂停/失败/完成边界均有测试；底部任务面板新增恢复收据列表、当前 lease 可识别/过期提示和安全清理按钮；当前已提供导入、批量章节、单章草稿、人物提取、审稿、只读修稿、定稿和架构生成八类 workflow-specific resume factory，均重新读取权威 SQLite/draft 来源并校验当前 lease，收据不保存正文或模型输出；完整跨工作流恢复审计、其他长任务（配置生成/目录生成等）和最终发布前人工验收仍待完成 |
