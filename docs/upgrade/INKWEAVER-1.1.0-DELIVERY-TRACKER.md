...
| G03 | 安装包与分发 | 13 | Windows 和两种 macOS 架构同 SHA 资格通过吗？ | 开发中 | 已回读 Windows 资格计划：`test → prepare:native-node → clean:build → build:win:artifacts → verify update/package → smoke app/installer/upgrade → restore-native-node → final`；Windows release-contract 测试现为 22 passed/1 skipped，冷启动与 monitor 确认窗口已加固；开发版 `0.9.2` 已实际生成并通过 `verify:win-update-artifacts`、`verify:win-package`、`smoke:win-app`、`smoke:win-installer`，但 v0.2.5 官方升级输入缺失，且该资产不是 1.1.0 冻结产物；macOS ARM64/x64 workflow 与统一提升合同已存在 |
