...
| C03 | 证据预览与合并 | 03 | 未提及字段不编造吗？候选确认前不改变事实吗？ | 开发中 | `417dfc8`：候选持久化和状态 IPC；`93ab074`：编辑器证据预览、接受/拒绝状态和浏览器测试；`3c40df9`：别名匹配、证据字段合并和现有 roster revision 原子提交；`cc0e26d`：带证据关系候选和关系闭包过滤；成长弧、字段级完整差异仍待完善 |
