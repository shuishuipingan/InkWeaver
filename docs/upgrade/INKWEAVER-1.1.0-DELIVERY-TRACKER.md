...
| --- | --- | --- | --- | --- | --- |
| A01 | 章节交接记录 | 04 | 现场和情绪有原文证据吗？来源修订后是否过期？ | 开发中 | `7ba46f6`：source-bound handoff repository/IPC；`de9d1dd`：structured extraction command；`d76f852`：定稿后非阻塞候选步骤；`257df5d`/`98db05e`：编辑审核面板/浏览器测试；`eed0921`：新 finalized draft 使旧交接失效；ChapterHandoffPanel 现在显示来源指纹、现场、视角、在场人物、即时目标、情绪、限制、承接方式、未完成动作、问题和证据；`scripts/quality-fixtures.ts`、`BLIND-QUALITY-REVIEW-PACKET.md` 和 `quality-review-summary.ts` 已建立 24 对章节盲评输入、私有映射与门槛汇总器；真实长篇阅读验收仍未完成 |
...
| A06 | 情绪与人物成长 | 06 | 重大事件影响是否延续？重建是否保留作者确认？ | 开发中 | 工作单记录前置情绪、触发、选择、代价、后续状态和正文证据；写前摘要会提醒上一章记录过 `nextState` 但本章尚未补记的人物余波；`4a0fbe5` 增加按章节排序的只读“人物成长轨迹”，展示触发、选择、代价、状态变化和证据；模型候选/历史重建的人工确认链与长篇质量评阅仍待完成 |
| A07 | 读者期待与多视角 | 06 | 切线落点和待回应问题是否保留？秘密是否串台？ | 开发中 | 工作单记录读者期待状态/延后理由/证据与视角最后落点、未解钩子、读者知识；现在可编辑期待提出章节、预计推进、状态、期限和明确延后理由；写前摘要新增叙事线沉寂/逾期提醒和“本章到期读者期待”提醒（来自前章工作单中未解决且到期/已逾期的期待）；`a622847` 增加按章节排序的只读“读者知识账本”，明确与角色知情边界分离；`0efb551` / `JOURNEY-J02-RECEIPT.md` 验证夜间悬念切换视角后回到原视角，另一条视角线的已确认秘密不会进入当前知情边界；独立读者知识/多视角长篇评阅仍待完成 |
...
| B01 | 分层上下文 | 07 | 早期关键事实能召回吗？省略和预算是否可解释？ | 开发中 | `5c498f0`：完整条目预算选择与 privacy-safe ContextReceipt；写稿上下文和 AI 输出面板显示纳入/省略原因；章节生成现在把固定规则、角色状态、活跃叙事线、知识库检索、确认交接和知情范围也登记为不含正文的来源层；AI 输出收据按来源层分组显示纳入/省略与预算原因；固定 100 章合成样本和召回协议已记录在 `docs/upgrade/INKWEAVER-1.1.0-QUALITY-SAMPLE-SET.md`，仍待实际召回率验收 |
| B02 | 历史状态与时间线 | 05 | 不同章节能查询不同有效状态吗？倒叙处理正确吗？ | 开发中 | `2503bda`：连续性事实支持 validFromChapter/validUntilChapter，写稿上下文和一致性检查过滤失效事实；`f3f2d2d` 在连续性工作单增加只读“查询历史状态章节”控件，按所选章节过滤有效事实并保留来源证据；故事时间/章节顺序的专门倒叙 UI 与长篇查询评测仍待完成 |
| B03 | 知情范围 | 05 | 世界事实、读者知识、角色知识和误信是否独立？ | 开发中 | 新增 source-bound `knowledge_events`，区分事实/信念/传闻、误信、获知方式、证据、有效章节和确认状态；写稿只注入当前角色已确认知识；连续性工作单另读 review-only 候选列表，作者确认/拒绝后才改变 confirmed 投影；`a622847` 增加按章节的读者知识账本，避免把读者已知误当作角色已知；`0efb551` 修复连续性面板按当前章节蓝图人物查询知情事件，避免多视角回切时把全项目 roster 的秘密串入当前角色边界；模型候选提取、独立知识审计和长篇泄漏评测仍待完成 |
...
| B05 | 改稿影响与重建 | 08 | 直接依赖失效吗？重建可恢复且不覆盖人工计划吗？ | 开发中 | `388d6c2`：纯函数收集历史改稿对后续连续性投影、下一章交接和叙事线索计划的影响章节；`ec53c38`：主进程列表 IPC、编辑器影响清单和选择状态/浏览器回归；`007d0e7` / `JOURNEY-J06-RECEIPT.md` 新增 source-bound `post_process` 重建工作流、按章节去重排序、暂停/恢复安全元数据和底部任务面板恢复入口；真实 provider 长篇重建评阅仍待完成 |
| C01 | 全文人物提取 | 03 | 后半段、跨块代词、多章和导入文本是否都覆盖？ | 开发中 | `43e5da1`：定稿人物状态提取保留开头+结尾有界窗口；`f03503c`：source-bound 全文分块计划；`f94a927`：证据字段解析、跨块别名合并和冲突标记；`edd74ce`/`8ff286f`：冻结模型租约的多分块提取工作流与入口；`ca02c27`：候选应用后状态闭环；新增人物候选面板来源覆盖摘要（来源类型、章节范围、privacy-safe 指纹）并有浏览器回归；稳定身份和同名作者选择仍待完善 |
...
| C03 | 证据预览与合并 | 03 | 未提及字段不编造吗？候选确认前不改变事实吗？ | 开发中 | `417dfc8`：候选持久化和状态 IPC；`93ab074`：编辑器证据预览、接受/拒绝状态和浏览器测试；`3c40df9`：别名匹配、证据字段合并和现有 roster revision 原子提交；`cc0e26d`：带证据关系候选和关系闭包过滤；`ca02c27`：应用后标记 applied；新增字段级勾选，应用只提交作者选中的字段；新增角色名单字段级差异纯函数（added/removed/changed、关系增删计数），并在候选应用成功后以摘要提示本次各角色改动的字段与关系数量；`CharacterExtractionCandidatesPanel` 新增可折叠“人物成长弧与状态”审阅区并有浏览器回归；全文成长弧回顾与长篇评测仍待完成 |
| D01 | 标签定位修复 | 02 | 标签始终靠近所属边吗？缩放拖拽后命中正确吗？ | 待验收 | `5584eec`：bounded local label layout；4 unit tests（含 200 边密集样本下每个可见标签到所属边距离不超过 maxOffset 的自动化断言）、RelationshipGraph browser 23 tests、typecheck 已通过；新增真实 Chromium Canvas 2D 证据测试 `src/components/editor/__tests__/RelationshipGraph.label-hit.browser.tsx`：单关系图在缩放和拖拽事件后，标签基线到所属边线段距离 ≤25 content px；更多关系密度/目标硬件评阅仍待完成 |
| D02 | 图谱密度和交互 | 09 | 大图谱能搜索过滤和聚焦吗？键盘/列表替代可用吗？ | 开发中 | `a2d023c`：姓名搜索、关系类型过滤、一跳邻域显示；`c03dc37`：别名搜索、可选二跳聚焦、键盘/列表替代和纯模型测试/浏览器回归；202 节点浏览器回归已通过画布尺寸/四象限绘制检查，新增 5000 节点图模型搜索/过滤/聚焦自动化压力测试；`439c6e9` 新增真实 Chromium 性能证据测试 `src/components/editor/__tests__/RelationshipGraph.performance.browser.tsx` 与基线文档 `INKWEAVER-1.1.0-GRAPH-PERFORMANCE-BASELINE.md`：当前 Windows 11 / Chrome 152 / i7-14650HX 记录首次可交互 45.6 ms、布局 RAF p95 16.8 ms、拖拽 RAF p95 16.7 ms、Long Task 0；跨硬件、500+ 节点真实项目复测仍待完成 |
| D03 | 有向多关系 | 09 | 单向情感和反向信任能分别呈现并追踪证据吗？ | 开发中 | 角色关系契约/解析器已保留 direction、sourceChapter、evidence；图谱提示条显示箭头、来源章节和证据，线条仍按配对合并；`8a20fe9` 新增可键盘访问的折叠“关系历史与证据”面板，按来源章节展示方向、短标签和正文证据；`RelationshipGraph` 现在可通过项目会话打开对应定稿章节的证据，浏览器回归覆盖；真实长篇关系变迁评阅仍待完成 |
| D04 | 布局与关系历史 | 09 | 固定位置重开保持吗？历史关系不会读取未来状态吗？ | 开发中 | 图谱展示状态新增项目级布局存储、固定/取消固定和重置布局；布局操作不写角色事实；关系历史面板支持“截至章节”只读筛选，并新增两个章节的新增/不再出现关系摘要，按 sourceChapter 隐藏未来关系且有浏览器回归；关系解析与提示详情仍保留同标签但不同来源章节/证据的历史条目；完整章节关系事件时间轴和真实长期关系评阅仍待完成 |
