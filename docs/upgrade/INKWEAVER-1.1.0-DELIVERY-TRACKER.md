...
| E01 | 伏笔兑现 | 10 | 计划、推进、回收有证据吗？超期可以解释和调整吗？ | 通过 | `ACCEPTANCE-E01-E09-RECEIPT.md`：伏笔状态、逐字证据、沉寂/逾期、延后理由、跨卷摘要和候选审核通过；模型兑现质量按发布范围豁免 |
| E02 | 写前准备 | 07 | 当前蓝图、交接、规则、人物和缺失信息可查看吗？ | 通过 | `ACCEPTANCE-E01-E09-RECEIPT.md`：蓝图、交接、人物、规则、活跃叙事线、知情边界和缺失资料摘要通过 |
| E03 | 写后审查 | 07 | 衔接、一致性、重复和节奏可分类处理吗？ | 通过 | `ACCEPTANCE-E01-E09-RECEIPT.md`：相邻衔接、确定性冲突、人物候选、重复提示、审查问题编辑/忽略/确认和修稿入口通过；模型节奏语义按发布范围豁免 |
| E04 | 安全局部修稿 | 08 | 锁定段保留吗？应用前比较正文版本吗？ | 通过 | `ACCEPTANCE-E01-E09-RECEIPT.md`：正文 fingerprint、段落锁定、三栏合并和旧 Proposal 拒绝通过 |
| E05 | 任务恢复 | 08 | 取消、重启、重复请求是否不造成重复或错误写入？ | 通过 | `ACCEPTANCE-E01-E09-RECEIPT.md`：取消/暂停/失败/重启、safe checkpoint、12 类恢复 factory、lease 拒绝和幂等资源声明通过 |
| E06 | 模型成本 | 07 | 模型与预算明确吗？缺失用量不会显示成零吗？ | 通过 | `ACCEPTANCE-E01-E09-RECEIPT.md` 与 `REAL-PROVIDER-QUALIFICATION-DRY-RUN-RECEIPT.md`：模型/预算/usage/未知状态、汇总和三 adapter dry-run 通过；真实 provider 价格与成本按发布范围豁免 |
| E07 | 项目快照恢复 | 01、08 | 数据库和附件一致吗？恢复后原稿和引用完整吗？ | 通过 | `ACCEPTANCE-E01-E09-RECEIPT.md` 与 `JOURNEY-J08-RECEIPT.md`：SQLite backup、manifest/hash、附件复制、空目录隔离恢复和逐文件核验通过；真实跨平台人工演练按发布范围豁免 |
| E08 | 导出完整性 | 08 | 无蓝图原稿也能导出全部定稿且顺序正确吗？ | 通过 | `ACCEPTANCE-E01-E09-RECEIPT.md` 与 `JOURNEY-J08-RECEIPT.md`：finalized authority 枚举、无蓝图顺序、缺章/标题/字数/内容 SHA manifest、目录授权和逐字节回读通过 |
| E09 | 双语与可访问性 | 10 | 新入口、错误、审核和恢复流程都有双语与键盘支持吗？ | 通过 | `ACCEPTANCE-E01-E09-RECEIPT.md` 与 `ACCESSIBILITY-I18N-SMOKE-RECEIPT.md`：i18n scanner、38 files/225 browser tests、键盘列表/审核/恢复控件和错误状态通过；WCAG 全量人工审计按发布范围豁免 |
| F01 | DSH 兼容版本 | 11 | 核对的是官方默认分发渠道且固定准确版本吗？ | 通过 | 2026-09-11 npm 查询确认 `@deepseek-ai/dsh` latest=`0.1.5-rc.1`、next=`0.1.5-rc.2`、alpha=`0.1.5-alpha.2`；插件已将 DSH 家族锁定到 `0.1.5-rc.1`，并保留官方仍未发布 0.1.5 版本的两个 client pin；persona/SystemPrompt/Session 事件迁移与消息数组 system-prompt 兼容证据见 `plugins/inkweaver-dsh/docs/dsh-0.1.5-rc.1-compatibility.md`；当前冻结候选完整回归为 40 文件/433 passed/6 skipped，机器可读 qualification 收据与固定 SHA 见 `ACCEPTANCE-F04-F05-RECEIPT.md` |
