...
| D04 | 布局与关系历史 | 09 | 固定位置重开保持吗？历史关系不会读取未来状态吗？ | 开发中 | 图谱展示状态新增项目级布局存储、固定/取消固定和重置布局；布局操作不写角色事实；关系历史面板现在支持“截至章节”只读筛选，按 sourceChapter 隐藏未来关系并有浏览器回归；关系解析与提示详情仍保留同标签但不同来源章节/证据的历史条目；完整章节关系事件时间轴、两个章节比较和真实长期关系评阅仍待完成 |
