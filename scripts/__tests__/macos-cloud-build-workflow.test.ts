...
const vectorRunnerStubPath = path.join(repositoryRoot, 'scripts', 'release-vector-smoke-electron-stub.mjs')
...
describe('macOS ARM64 cloud build workflow contract', () => {
  ...
  it('uses a manual GitHub-hosted Apple Silicon qualification without Release publication', () => {
    ...
    const vectorRunnerStub = readRequired(vectorRunnerStubPath)
    ...
    expect(vectorRunnerStub).toContain('export const ipcMain')
