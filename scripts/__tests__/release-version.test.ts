...
describe('v1.2.5 release metadata', () => {
  ...
  it('resolves the release tag and exact seven-asset contract from the package version', () => {
    ...
    expect(`v${pkg.version}`).toBe('v1.2.5')
    expect(profile.releaseAssets.map(({ name }) => name.replaceAll('{version}', pkg.version))).toEqual([
      'inkweaver-setup-1.2.5.exe',
      'inkweaver-setup-1.2.5.exe.blockmap',
      'latest.yml',
      'inkweaver-mac-arm64-1.2.5-installer.dmg',
      'inkweaver-mac-arm64-1.2.5-installer.dmg.sha256',
      'inkweaver-mac-x64-1.2.5-installer.dmg',
      'inkweaver-mac-x64-1.2.5-installer.dmg.sha256',
