...
describe('v1.2.2 release metadata', () => {
  ...
  it('resolves the release tag and exact seven-asset contract from the package version', () => {
    ...
    expect(`v${pkg.version}`).toBe('v1.2.2')
    expect(profile.releaseAssets.map(({ name }) => name.replaceAll('{version}', pkg.version))).toEqual([
      'inkweaver-setup-1.2.2.exe',
      'inkweaver-setup-1.2.2.exe.blockmap',
      'latest.yml',
      'inkweaver-mac-arm64-1.2.2-installer.dmg',
      'inkweaver-mac-arm64-1.2.2-installer.dmg.sha256',
      'inkweaver-mac-x64-1.2.2-installer.dmg',
      'inkweaver-mac-x64-1.2.2-installer.dmg.sha256',
