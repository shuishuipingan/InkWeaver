...
describe('v1.2.14 release metadata', () => {
  it('uses the release version in package metadata', () => {
    const pkg = JSON.parse(readFileSync('package.json', 'utf8')) as { version: string }
    expect(pkg.version).toBe('1.2.14')
  })
  ...
  it('resolves the release tag and exact seven-asset contract from the package version', () => {
    ...
    expect(`v${pkg.version}`).toBe('v1.2.14')
    expect(profile.releaseAssets.map(({ name }) => name.replaceAll('{version}', pkg.version))).toEqual([
      'inkweaver-setup-1.2.14.exe',
      'inkweaver-setup-1.2.14.exe.blockmap',
      'latest.yml',
      'inkweaver-mac-arm64-1.2.14-installer.dmg',
      'inkweaver-mac-arm64-1.2.14-installer.dmg.sha256',
      'inkweaver-mac-x64-1.2.14-installer.dmg',
      'inkweaver-mac-x64-1.2.14-installer.dmg.sha256',
