...
describe('v1.2.3 release metadata', () => {
  ...
  it('resolves the release tag and exact seven-asset contract from the package version', () => {
    ...
    expect(`v${pkg.version}`).toBe('v1.2.3')
    expect(profile.releaseAssets.map(({ name }) => name.replaceAll('{version}', pkg.version))).toEqual([
      'inkweaver-setup-1.2.3.exe',
      'inkweaver-setup-1.2.3.exe.blockmap',
      'latest.yml',
      'inkweaver-mac-arm64-1.2.3-installer.dmg',
      'inkweaver-mac-arm64-1.2.3-installer.dmg.sha256',
      'inkweaver-mac-x64-1.2.3-installer.dmg',
      'inkweaver-mac-x64-1.2.3-installer.dmg.sha256',
