...
function releaseFixture(tag: string) {
  ...
  const installerName = `inkweaver-setup-${version}.exe`
...
function fixtureFetcher(fromTag: string, expectedTag: string, options: { latestTag?: string, corruptDigest?: boolean } = {}) {
  ...
  const fetcher = async (url: string | URL) => {
    ...
    requests.push(value)
    if (value === 'https://api.github.com/repos/shuishuipingan/InkWeaver/releases/latest') {
      return response({ ...expected.release, tag_name: latestTag })
    }
    if (value === `https://api.github.com/repos/shuishuipingan/InkWeaver/releases/tags/${fromTag}`) {
      return response(from.release)
    }
    if (value === `https://api.github.com/repos/shuishuipingan/InkWeaver/releases/tags/${expectedTag}`) {
