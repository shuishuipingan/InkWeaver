...
if ([string]::IsNullOrWhiteSpace($InstallerPath)) {
  $InstallerPath = Join-Path $root ("release\{0}\inkweaver-setup-{0}.exe" -f [string]$packageJson.version)
}
...
$uninstaller = Join-Path $installRoot 'Uninstall InkWeaver.exe'
$lastWindowSnapshot = @()
...
function Invoke-AiNovelPackagedVectorSmoke {
  ...
  try {
    ...
    Invoke-AiNovelMonitoredExecutable `
      -Path $Path `
      -Arguments @(
        ("--user-data-dir={0}" -f (Join-Path $smokeRoot 'packaged-vector-user-data')),
        "--ai-novel-release-smoke=$token"
      ) `
...
function Invoke-AiNovelPackagedOfficialHomepageSmoke {
  ...
  try {
    ...
    Invoke-AiNovelMonitoredExecutable `
      -Path $Path `
      -Arguments @(
        ("--user-data-dir={0}" -f (Join-Path $smokeRoot 'packaged-homepage-user-data')),
        "--ai-novel-release-homepage-smoke=$token"
      ) `
    ...
    $validEvidence = (
      ...
      $result.trustedIntent.url -eq 'https://github.com/shuishuipingan/InkWeaver' -and
...
function Invoke-AiNovelPackagedSkinSmoke {
  ...
  # installer smoke's isolated InkWeaver data home, never a user profile or caller path.
  $token = [guid]::NewGuid().ToString('N')
  ...
  try {
    ...
    Invoke-AiNovelMonitoredExecutable `
      -Path $Path `
      -Arguments @(
        ("--user-data-dir={0}" -f (Join-Path $smokeRoot 'packaged-skin-user-data')),
        "--ai-novel-release-skin-smoke=$token"
      ) `
